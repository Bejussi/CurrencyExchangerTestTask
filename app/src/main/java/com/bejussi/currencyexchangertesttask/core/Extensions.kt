...
fun Spinner.afterItemSelected(onItemSelected:() -> Unit) {
...
fun CurrencyResponceData.mapCurrencyResponceDataToCurrencyResponce(): CurrencyResponce {
    return CurrencyResponce(
        base = this.base,
        date = this.date,
        rates = this.ratesData.mapRatesDataToRates()
    )
}

fun RatesData.mapRatesDataToRates(): Rates {
    return Rates(
        AED = this.AED,
        AFN = this.AFN,
        ALL = this.ALL,
